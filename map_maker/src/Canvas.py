...
class Canvas:
  def __init__(self, screen: pygame.Surface):
    ...
    self.canvas = pygame.Surface((CANVAS_SIZE[0], CANVAS_SIZE[1]), pygame.SRCALPHA)
  ...
  def save_map(self):
    with open("C:/Users/mvhx/Documents/game-ip/data/settings.json", "w") as data:
      settings = {
        "tile_size": TILE_SIZE,
    ...
    with open("C:/Users/mvhx/Documents/game-ip/data/cache.json", "w") as cache:
      json.dump(self.object_position_list, cache, indent=2)

  def load_save(self):
    try:
      with open("C:/Users/mvhx/Documents/game-ip/data/cache.json") as data:
  ...
  def draw_grid(self):
    map = pygame.image.load(
      "C:/Users/mvhx/Documents/game-ip/map_maker/src/Mapa.png"
    ).convert_alpha()
    map = pygame.transform.scale(
      map, (CANVAS_SIZE[0] - BORDER * 2, CANVAS_SIZE[1] - BORDER * 2)
    )
    self.canvas.blit(map, (BORDER, BORDER))
    for l_idx, line in enumerate(self.saveless_grid):
      for c_idx, tile in enumerate(line):
        tile_x = (CANVAS_GAP * c_idx) + TILE_SIZE * c_idx
        tile_y = (CANVAS_GAP * l_idx) + TILE_SIZE * l_idx

        rect = pygame.draw.rect(
          self.canvas,
          screen_colors["tile_color"]
          if tile[0] < 0
          else pallet_colors[self.colors[tile[0]]],
        ...
        if pygame.Rect(tuple(col[1])).collidepoint(mouse_pos):
          ...
          if col[0] < 0:
            ...
            if clicked_slot not in self.object_position_list:
              ...
              pygame.draw.rect(
                self.canvas,
                pallet_colors[self.colors[col[0]]],
                clicked_slot[1],
              )
