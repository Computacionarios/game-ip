...
class Personagem(pygame.sprite.Sprite):
  ...
  def processar_efeitos(self):
    ...
    if self.lentidao > 0:
      self.lentidao -= 1
    if self.imune_paralisia > 0:
      self.imune_paralisia -= 1
    if self.imune_envenenamento > 0:
      self.imune_envenenamento -= 1
  ...
  def status_str(self):
    status = []
    if self.paralisia > 0:
      status.append(f"Paralisado({self.paralisia})")
    if self.envenenamento > 0:
      status.append(f"Envenenado({self.envenenamento})")
    if self.lentidao > 0:
      status.append(f"Lento({self.lentidao})")
    if self.imune_paralisia > 0:
      status.append(f"ImuneRaio({self.imune_paralisia})")
    if self.imune_envenenamento > 0:
      status.append(f"ImuneMetal({self.imune_envenenamento})")
...
def desenhar_personagem(tela, personagem, fonte_status):
  ...
  y_hp_texto = y_hp_barra - (
    altura_fonte // 4
  )  # Ajuste fino para alinhar texto e barra

  pygame.draw.rect(
    tela, (255, 0, 0), (personagem.rect.x, y_hp_barra, largura_barra, altura_barra)
  )  # Fundo
  pygame.draw.rect(
    tela,
    cor_hp,
    (
      personagem.rect.x,
      y_hp_barra,
      largura_barra * personagem.hp_pct,
      altura_barra,
    ),
  )  # Vida atual

  texto_hp = fonte_status.render(
    f"HP: {personagem.hp}/{personagem.HP_MAX}", True, (255, 255, 255)
  )
  ...
  pygame.draw.rect(
    tela, (50, 50, 50), (personagem.rect.x, y_mp_barra, largura_barra, altura_barra)
  )  # Fundo
  pygame.draw.rect(
    tela,
    (0, 150, 255),
    (
      personagem.rect.x,
      y_mp_barra,
      largura_barra * personagem.mp_pct,
      altura_barra,
    ),
  )  # MP atual
