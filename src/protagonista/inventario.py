...
class Inventario:
    def __init__(self):
        ...
        inventario_largura = (
            self.tamanho_slot + self.espacamento_itens
        ) * self.colunas + self.espacamento_itens
        inventario_altura = (
            (self.tamanho_slot + self.espacamento_itens) * self.linhas
            + self.espacamento_itens
            + self.espaco_player
        )
        self.superficie = pygame.Surface(
            (inventario_largura, inventario_altura)
        )  # Define largura e altura da superficie do inventário

        self.inventario_retangular = self.superficie.get_rect(
            center=(inventario_largura / 2, inventario_altura / 2)
        )  # Inventário definido como um retângulo no centro da tela (pode variar)

        self.sprites_cartas = {}
        self.sprite_player = None

        self.font = pygame.font.Font("\assets\fonts\main_font.ttf", 18)

        self.itens = []  # Os dicionários com os dados do item

        self.sprites_cartas = Coletavel().util()
        self.sprite_player = Player().parado(FRENTE)
    ...
    def desenhar_inventario(self, tela):
        ...
        tela.blit(
            self.superficie, self.inventario_retangular.topleft
        )  # Exibindo o inventário na tela
    ...
    def desenhar_slot_inventario(self):
        ...
        for linha in range(self.linhas):  # Percorrendo linhas
            for coluna in range(self.colunas):  # Percorrendo colunas
                slot_x = (
                    coluna * (self.tamanho_slot + self.espacamento_itens)
                    + self.espacamento_itens
                )
                slot_y = (
                    linha * (self.tamanho_slot + self.espacamento_itens)
                    + self.espacamento_itens
                    + self.espaco_player
                )
                pygame.draw.rect(
                    self.superficie,
                    cores["cinza"],
                    (slot_x, slot_y, self.tamanho_slot, self.tamanho_slot),
                )

                if (
                    linha == self.linha_selecionada
                    and coluna == self.coluna_selecionada
                ):
                    pygame.draw.rect(
                        self.superficie,
                        cores["amarelo"],
    ...
    def desenhar_itens_slot(self):
        ...
        for indice, item in enumerate(self.itens):
            ...
            quantidade_texto = self.font.render(
                str(item["quantidade"]), True, cores["branco"]
            )
    ...
    def desenhar_menu_opcoes(self, tela):
        ...
        menu_retangular = pygame.Rect(
            menu_x,
            menu_y,
            menu_largura,
            menu_altura * (len(menu_opcoes)) + self.espacamento_itens * 1.5,
        )
        ...
        for i, opcoes in enumerate(menu_opcoes):
            opcoes_rect = pygame.Rect(
                menu_x + self.espacamento_itens,
                menu_y + self.espacamento_itens + i * menu_altura,
                menu_largura - (self.espacamento_itens * 2),
                menu_altura - self.espacamento_itens / 2,
            )  # O sitens do menu são variaveis, essa função aceita a troca sem interferir
            ...
            tela.blit(
                texto_supercie,
                (
                    menu_x + self.espacamento_itens * 2,
                    menu_y + (menu_altura / 2) + (i * menu_altura),
                ),
            )
        ...
        if self.sprite_player:
            pygame.draw.rect(
                ...
                (
                    self.espacamento_itens,
                    self.espacamento_itens,
                    self.inventario_retangular.width,
                    self.player_altura,
                ),
            )

            sprite_x = self.inventario_retangular.width / 2
            sprite_y = self.player_altura / 2
            self.superficie.blit(self.sprite_player, (sprite_x, sprite_y))
    ...
    def adicionar_itens(self, id, sprite, nome, quantidade=1):
        ...
        self.itens.append(
            {"id": id, "sprite": sprite, "nome": nome, "quantidade": quantidade}
        )
        ...
        if evento.type == pygame.KEYDOWN:
            ...
            if self.menu_aberto:
                ...
                # Seta para baixo no menu
                    ...
                    self.opcao_menu_atual = min(
                        len(self.menu_opcoes) - 1, self.opcao_menu_atual + 1
                    )
                ...
                # Seta para direita
                    ...
                    self.coluna_selecionada = min(
                        self.colunas - 1, self.coluna_selecionada + 1
                    )
                ...
                # Seta para baixo
                    ...
                    self.linha_selecionada = min(
                        self.linhas - 1, self.linha_selecionada + 1
                    )

                # Se clicar enter seleciona a carta e exibi menu de opções
                elif evento.key == pygame.K_RETURN:
                    indice = (
                        self.linha_selecionada * self.colunas + self.coluna_selecionada
                    )

                    if indice < len(self.itens):
                        slot_x = (
                            self.coluna_selecionada
                            * (self.tamanho_slot + self.espacamento_itens)
                            + self.espacamento_itens
                        )
                        slot_y = (
                        ...
                        self.menu_posicao = (
                            self.inventario_retangular.left
                            + slot_x
                            + self.tamanho_slot
                            + 10,
                            self.inventario_retangular.top + slot_y,
                        )
