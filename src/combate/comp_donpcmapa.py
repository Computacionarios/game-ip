...
class comp_donpcmapa:
  def __init__(self, tela, velocidade, percurso, npc_larg_hitbox, npc_alt_hitbox):
    ...
    # Criação da hitbox
    self.NPChitbox = pygame.Rect(
      self.percurso[self.index_doponto][0],
      self.percurso[self.index_doponto][1],
      self.npc_larg_hitbox,
      self.npc_alt_hitbox,
    )
  ...
  def atualizar(self, player):
    ...
    if self.NPChitbox.colliderect(player):
      ...
      if (
        self.NPChitbox.x == self.percurso[self.index_doponto][0]
        and self.NPChitbox.y == self.percurso[self.index_doponto][1]
      ):
    ...
    if estado_jogo == "EXPLORACAO":
      ...
      if not em_transicao:
        ...
        if teclas[pygame.K_LEFT] or teclas[pygame.K_a]:
          jogador_mapa.x -= vel_jogador
        if teclas[pygame.K_RIGHT] or teclas[pygame.K_d]:
          jogador_mapa.x += vel_jogador
        if teclas[pygame.K_UP] or teclas[pygame.K_w]:
          jogador_mapa.y -= vel_jogador
      ...
      renderizar_batalha(
        tela, instancia_batalha, LARGURA, ALTURA, fonte_log, fonte_carta_desc
      )
